...
import OutOfStockTable, { type GroupedOutOfStockProducts } from '@/components/inventory/OutOfStockTable';
...
import { PackageX, Lock, PackageOpen, Search, Barcode } from 'lucide-react';

/**
 * La página OutOfStockPage muestra todos los productos que están actualmente agotados
 * en todos los puntos de venta accesibles para el usuario actual, agrupados por marca y filtrables por búsqueda.
 *
 * @returns {JSX.Element} La interfaz de usuario de la página de productos agotados.
 */
export default function OutOfStockPage(): JSX.Element {
  ...
  /**
   * Objeto memoizado de productos agotados, agrupados por marca y filtrados por searchTerm.
   * Itera a través de los puntos de venta accesibles y su inventario para encontrar productos con cantidad 0.
   */
  const outOfStockProductsByBrand = useMemo((): GroupedOutOfStockProducts => {
    const groupedProducts: GroupedOutOfStockProducts = {};
    ...
    const sortedGroupedProducts: GroupedOutOfStockProducts = {};
  ...
  if (accessiblePointsOfSale.length === 0 && currentUser && currentUser.id !== 'user-0') {
    return (
      <div className="space-y-8">
        <h1 className="text-3xl font-bold text-foreground mb-6">Productos Agotados</h1>
        <Card className="w-full text-center shadow-none border-dashed">
          <CardHeader>
            <Lock className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
            <CardTitle>Sin Acceso a Ubicaciones</CardTitle>
            <CardDescription>
              {`El usuario "${currentUser.name}" no tiene acceso a ninguna ubicación de inventario para verificar productos agotados.`}
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-foreground">Productos Agotados</h1>
        <PackageX className="h-8 w-8 text-destructive" />
      </div>
      <p className="text-muted-foreground">
        Lista de productos actualmente sin stock. Puedes buscar por nombre, código o marca.
      </p>
      ...
      <OutOfStockTable productsByBrand={outOfStockProductsByBrand} searchTerm={searchTerm} totalInitially={totalProductsInitiallyOutOfStock} />
    </div>
  );
}
